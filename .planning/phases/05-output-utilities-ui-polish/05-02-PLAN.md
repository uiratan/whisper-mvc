...
3. **Store audio URL after transcription**: In the xhr 'load' handler (around line 142), after setting transcription, verify backend response structure and set audioUrl:
   - FIRST: Check `/api/upload/route.ts` response structure to confirm available fields
   - The API currently returns: `{success, message, transcription}` without filePath or fileName
   - REQUIRED: Verify if selectedFile (the File object from upload) is still available in scope
   - If selectedFile is available: Create object URL: `setAudioUrl(URL.createObjectURL(selectedFile))`
   - If not available: Log warning and skip player (player won't render without audioUrl)
   - NOTE: Object URLs should be revoked when component unmounts or new file is selected (add cleanup in useEffect)
